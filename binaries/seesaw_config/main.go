...
var (
	...
	caCertFile     = flag.String("ca-cert", "/etc/seesaw/ssl/ca/cert.pem", "the file path to CA certs file")
	serverCertDir  = flag.String("server-cert-dir", "/etc/seesaw/ssl/config/", "The directory where server certs and key are stored. Assuming file names are 'cert.pem' and 'key.pem'.")
)
